...
**Not deployed:** the compat-box chip in `product-enhancements.js` (label from the accessory's first axis). It is linked with a static tag by 44 fichas, so busting it means a global bump + full HTML deploy — ship it with the next bump.

**Migration status (Aug 2026):** stages 1–3 done — `variant-pop.js`, `product-enhancements.js` and all 12 fichas are consumers; stage 5 (cart/checkout presentation) done and live. Pending: multi-axis accessories (+ categories/compatibility), **backend named attributes** (`cartItemsPayload` in `pago.js` still ships only `color`/`colorLabel`, so a stored order has no `attrs` and server-rendered text — emails, admin — still says "Color"), legacy cleanup, `products-server.js` mirror.
